"""
Script 7: CDR-H3 loop RMSD computation
Input: benchmark_master.tsv, pdb_structures/, Google Drive (predicted CIFs via rclone)
Output: boltz2_cdr_h3_rmsd.tsv

For each of 304 complexes x 5 seeds:
1. Downloads predicted mmCIF from Google Drive via rclone
2. Superimposes predicted structure onto native using framework residues
3. Computes CDR-H3 loop RMSD after framework superposition

Key finding: Mean CDR-H3 RMSD 4.87A vs framework 2.78A
CDR-H3 RMSD varies 6.9x more than ipTM across seeds (conformational multiplicity).

Requirements: pip install biopython gemmi && brew install rclone
"""
...
TMP = '/tmp/boltz_cifs'
OUTFILE = 'boltz2_cdr_h3_rmsd.tsv'
...
if os.path.exists(OUTFILE):
    ...
    print(f"Resuming from checkpoint: {len(results)} results already saved")
else:
    done = set()
    results = []

failed = []
total = len(df) * len(SEEDS)
count = len(results)

print(f"Starting CDR-H3 RMSD — {total - count} remaining")
...
for i, (_, row) in enumerate(df.iterrows()):
    ...
    if not isinstance(cdr_h3_seq, str) or len(cdr_h3_seq) == 0:
        continue
    ...
    for seed in SEEDS:
        ...
        try:
            if not os.path.exists(cif_dst):
                r = subprocess.run([
                    'rclone', 'copy',
                    f'{DRIVE_OUT}/{pdb_id}/seed{seed}/{pdb_id}_model_0.cif',
                    TMP
                ], capture_output=True, timeout=90)
                src = f'{TMP}/{pdb_id}_model_0.cif'
            ...
            if count % 100 == 0:
                print(f"[{count}/{total}] {pdb_id} seed {seed}: CDR-H3 RMSD={cdr_h3_rmsd:.3f}A")
            ...
            failed.append((pdb_id, seed, str(e)))
